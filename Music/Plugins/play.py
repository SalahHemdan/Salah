...
@Client.on_message(command(["/play", "صلاح" ,"الماس" ,"احتلال" ,"شغل" ,"تشغيل", f"/play@{BOT_USERNAME}", "تشغيل"]))
async def play(_, message: Message):
    chat_id = message.chat.id
    if message.sender_chat:
        return await message.reply_text(
            """
عارف ان حلم الاعضاء يشغلو اغاني بس مش هينفع 😢
"""
        )
...
قم بالغاء الحظر للاستخدام البوت بنجاح
"""
            )
            return
...
`تشغيل او شغل او صلاح + أسم المقطع الصوتي 🎧`
""",
            )
            return
        what = "Query Given"
